...
def main(args):
    ...
    in_dims = [features.shape[1] for features in features]
    features = [feat.to('cpu') for feat in features]
    labels = labels.to('cpu')
    G = [g.to('cpu') for g in G]
    ADJ = ADJ.to('cpu')
    model = OSGNN_GL(in_dims, args['hidden_units'], len(G), args['dropout'], num_classes).to('cpu')

    G = [graph.to('cpu') for graph in G]
    ...
    print('\ntesting...')
    model.load_state_dict(torch.load('checkpoint/checkpoint_{}.pt'.format(args['dataset']), map_location='cpu'))
    model.eval()
    logits, h, _ = model(features, G, ADJ, type_mask, norm, args['agg'])
    evaluate_results_nc(logits[test_idx].detach().cpu().numpy(), labels[test_idx].cpu().numpy(), int(labels.max()) + 1)
